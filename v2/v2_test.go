...
import (
	parser "gitlab.33.cn/proof/parser"
	"reflect"
	"testing"
)
...
func TestNewProofV2(t *testing.T) {
	type args struct {
		data string
	}
	tests := []struct {
		...
		{"Testdata1",args{data:Testdata1},&ProofV2{TxData:  Testdata1}},
		{"Testdata2",args{data:Testdata2},&ProofV2{TxData:Testdata2}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewProofV2(tt.args.data); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewProofV2() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestProofV2_ToTx(t *testing.T) {
	type fields struct {
		TxData string
	}
	tests := []struct {
...
func TestProofV2_ToView(t *testing.T) {
	type fields struct {
		TxData string
	}
	tests := []struct {
